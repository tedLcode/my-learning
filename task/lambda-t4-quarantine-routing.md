# Task Summary — BDI Lambda Trigger: Quarentine Task (new one not old one updated)
...
| 15 | Added **quarantine routing**: files landing under `quarantine/hc/{yyyy}/{mm}/{dd}/` path pattern are routed to job `AP178041QUFMON` via regex `_QUARANTINE_PATH_RE = re.compile(r'^quarantine/hc/\d{4}/\d{2}/\d{2}/')` — validated with dedicated quarantine test files under `s3_test_files/quarantine/` |
